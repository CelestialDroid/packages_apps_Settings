...
import com.android.settings.celestial.util.CelestialUtils;
...
public class ConstraintsHelper {
    ...
    public ConstraintsHelper(Context context, AttributeSet attrs, Preference pref) {
        ...
        TypedArray a = context.getResources().obtainAttributes(attrs,
                R.styleable.celestial_SelfRemovingPreference);
        mReplacesKey = a.getString(R.styleable.celestial_SelfRemovingPreference_replacesKey);
    ...
    private void checkIntent() {
        ...
        if (i != null) {
            if (!CelestialUtils.resolveIntent(mContext, i)) {
                Graveyard.get(mContext).addTombstone(mPref.getKey());
                mAvailable = false;
            }
        }
    ...
    private boolean checkConstraints() {
        ...
        try {
            ...
            boolean rOwner = a.getBoolean(R.styleable.celestial_SelfRemovingPreference_requiresOwner, false);
            if (rOwner && UserHandle.myUserId() != UserHandle.USER_OWNER) {
                return false;
            }

            // Check if a specific package is installed
            String rPackageInst = a.getString(R.styleable.celestial_SelfRemovingPreference_requiresPackageInstalled);
            if (rPackageInst != null) {
                boolean negated = isNegated(rPackageInst);
                if (negated) {
                    rPackageInst = rPackageInst.substring(1);
                }
                boolean available = CelestialUtils.isPackageInstalled(mContext, rPackageInst);
            ...
            String rPackageEnb = a.getString(R.styleable.celestial_SelfRemovingPreference_requiresPackageEnabled);
            if (rPackageEnb != null) {
                boolean negated = isNegated(rPackageEnb);
                if (negated) {
                    rPackageEnb = rPackageEnb.substring(1);
                }
                boolean available = CelestialUtils.isPackageEnabled(rPackageEnb, mContext);
                if (available == negated) {
                    return false;
                }
            }
            ...
            String rPackageAvl = a.getString(R.styleable.celestial_SelfRemovingPreference_requiresPackageAvailable);
            if (rPackageAvl != null) {
                boolean negated = isNegated(rPackageAvl);
                if (negated) {
                    rPackageAvl = rPackageAvl.substring(1);
                }
                boolean available = CelestialUtils.isPackageAvailable(rPackageAvl, mContext);
                if (available == negated) {
                    return false;
                }
            }
            ...
            String rAction = a.getString(R.styleable.celestial_SelfRemovingPreference_requiresAction);
            if (rAction != null) {
                boolean negated = isNegated(rAction);
                if (negated) {
                    rAction = rAction.substring(1);
                }
                boolean available = CelestialUtils.resolveIntent(mContext, rAction);
                if (available == negated) {
                    return false;
                }
            }
            ...
            String rProperty = a.getString(R.styleable.celestial_SelfRemovingPreference_requiresProperty);
            if (rProperty != null) {
            ...
            TypedValue tv = a.peekValue(R.styleable.celestial_SelfRemovingPreference_requiresConfig);
            if (tv != null && tv.resourceId != 0) {
                if (tv.type == TypedValue.TYPE_STRING &&
                    ...
                    int mask = a.getInt(
                            R.styleable.celestial_SelfRemovingPreference_requiresConfigMask, -1);
